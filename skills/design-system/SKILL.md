...
license: MIT
metadata:
  author: BuilderOS (modified for NythanBuilds pipeline)
  version: "1.1-nb"
...
If the founder asks for any of the above, tell them it's outside this skill's scope and that their own pipeline handles it. Do not offer to do it anyway.
...
- **Live website URLs** — if the Chrome DevTools MCP is available, use it to open the URL and take a screenshot for analysis. Otherwise ask the founder to paste a screenshot, or use `WebFetch` to read content/styles only as a supplementary signal (not the primary visual source).
...
>
> Open `docs/design.html` in your browser now and check the colours, type scale, and components look right before any slices get built against them. Fixing a token now is free; fixing it after twelve slices have implemented against it is not."
...
> "Next step is yours: run `/to-tickets` to slice the work, and every UI step will implement against these tokens."
...
- **A build step needs a token that doesn't exist** — Add it here first, in both files, then let the step use it. Never let a step hardcode a one-off value.
